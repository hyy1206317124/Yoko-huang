...
<!--	<div class="text-c">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20">
            <span class="l">
                ...
                <a class="btn btn-primary radius" onclick="picture_add('添加内容','<?php echo U('Image/image_add');?>')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加图片</a>
            </span> 
		...
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			<thead>
				...
				<tr class="text-c" id="ad_id<?php echo $ro['id'];?>">
					...
					<td><a href="javascript:;" ><img width="180" class="picture-thumb" height="100" src="/saide.<?php echo ($ro['image']); ?>"></a></td>
                                        <td><?php echo ($ro['abstract']); ?></td>
					<td><?php echo (time_date($ro['time'])); ?></td>
                                       <?php if($ro['status'] == 1){ ?>
                                        <td class="td-status"><span class="label label-success radius" flag="<?php echo $ro['status'];?>" onClick="picture_stop(this,<?php echo ($ro['id']); ?>)">已启用</span></td>
                                        <?php }elseif($ro['status'] == -1){ ?>
                                        <td class="td-status"><span class="label radius" flag="<?php echo $ro['status'];?>" onClick="picture_stop(this,<?php echo ($ro['id']); ?>)">已停用</span></td>
                                        <?php } ?>
					<td class="td-manage">
                                            <a style="text-decoration:none" class="ml-5" onClick="picture_edit('图库编辑','<?php echo U("Image/image_update?id=".$ro["id"]);?>','10001')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a> 
...
function picture_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: '<?php echo U("Image/image_delete");?>',
                        data:{id:id},
			dataType: 'json',
			success: function(data){
...
function datadel(){
    ...
    layer.confirm('确认要删除吗？',function(index){
        ...
        $.ajax({
            url:"<?php echo U('Image/image_delete_all');?>",
            data:{id:id_str},
            type:'post',
            dataType:'json',
