...
<div class="page-container">
	<form class="form form-horizontal" id="form-article-add">
            <input type="hidden" name="information_image" id="information_image" value="<?php echo ($row['information_image']); ?>" >
             <input type="hidden" name="thumb_path" >
              <input type="hidden" name="id" id="id" value="<?php echo ($row['id']); ?>">
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>标题：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo ($row['information_title']); ?>" placeholder="" id="information_title" name="information_title">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">排序值：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo ($row['information_page']); ?>" placeholder="" id="information_page" name="information_page">
			</div>
		</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
                            <textarea name="information_abstract" id="information_abstract" cols="" rows="" class="textarea"  placeholder="说点什么...最少输入10个字符" datatype="*10-100" dragonfly="true" nullmsg="备注不能为空！" onKeyUp="$.Huitextarealength(this,200)">
                                <?php echo ($row['information_abstract']); ?>
                            </textarea>
			</div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<div class="uploader-thum-container">
					<div id="fileList" class="uploader-list"><img height="100" src="/saide.<?php echo ($row['image']); ?>"></div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9"> 
                            <textarea id="information_body" name="information_body" style="width:100%;height:400px;"><?php echo ($row['information_body']); ?></textarea> 
			</div>
		</div>
		<div class="row cl">
...
<script type="text/javascript">
    $(function(){
       $('#but').click(function(){
          var uid = $('#uid').val();
          var information_image = $('#information_image').val();
          var information_page = $('#information_page').val();
          if(!/^[0-9]*$/.test(information_page)){
             layer.msg('排序值请输入数字');
             return false;
          }
          if(information_page == ''){
          ...
          var information_abstract = $('#information_abstract').val();
           if(information_abstract.length<10 || information_abstract.length>200){
                layer.msg('请输入10到200的内容摘要',{icon: 5,time:1000});
                return false;
            }
          var information_title =$('#information_title').val();
          if(!/^[0-9A-Za-z\u4e00-\u9fa5]{0,300}$/.test(information_title)){
          ...
          var information_body=UE.getEditor('information_body').getContent();
          if(information_body == ''){
              layer.msg("请输入内容");
              return false;
          }
          var id =$('#id').val();
          var url="<?php echo U('Information/information_doUpdate');?>";
          var data={id:id,information_image:information_image,information_page:information_page,information_abstract:information_abstract,information_body:information_body,information_title:information_title};
          var dataType='json';
          $.post(
                url,
                data,
    ...
    $(function(){
        var ue = UE.getEditor('information_body',{
            'enterTag' : 'br' 
        });
        /*init webuploader*/  
        ...
        var uploader = WebUploader.create({  
            ...
            // 文件接收服务端。  
            server: "<?php echo U('Admin/Information/adver_upload');?>",  
        ...
        uploader.on('uploadSuccess', function( file, res) {  
            if(res.image){
                 //alert(res.img_path);
                 $("input[name='information_image']").val(res.image);
