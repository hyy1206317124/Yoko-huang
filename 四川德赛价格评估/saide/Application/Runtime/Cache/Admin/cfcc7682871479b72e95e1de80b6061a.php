...
<div class="page-container">
	<form class="form form-horizontal" id="form-article-add">
	     ...
	     <input style="display: none" type="text" value="<?php echo ($row['id']); ?>" id="id" name="id">
	
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>图片标题：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo ($row['title']); ?>" placeholder="title" id="title" name="title">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">图片作者：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo ($row['author']); ?>" placeholder="author" id="author" name="author">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">图片来源：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="<?php echo ($row['source']); ?>" placeholder="source" id="source" name="source">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">图片摘要：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<textarea name="abstract" id="abstract" cols="" rows="" class="textarea"  placeholder="说点什么...最少输入10个字符" datatype="*10-100" dragonfly="true" nullmsg="备注不能为空！" onKeyUp="$.Huitextarealength(this,200)"><?php echo ($row["abstract"]); ?></textarea>
			</div>
		</div>
                <div class="row cl">
                            ...
                            <div class="formControls col-xs-8 col-sm-9">
                                    <div class="uploader-thum-container">
                                            ...
                                            <button id="btnstar" name="image" type="button" class="btn btn-default radius ml-10">开始上传</button>
                                    </div>
                            </div>
                    </div>
<!--		<div class="row cl">
...
$(function(){  
   ...
   var uploader = WebUploader.create({  
       ...
       // 文件接收服务端。  
       server: "<?php echo U('admin/Image/adver_upload');?>",  
...
$(function(){
    $('#sub').click(function(){
        var url = '<?php echo U("Image/image_updates");?>';
        var data = {};
        data['id'] = $('#id').val();
        ...
        $.post(
                ...
                function(d){
                    if(d.code == 0){
                        layer.msg(d.msg);
                        location.href='<?php echo U("Image/lists");?>';
                    }
                    if(d.code == 1){
                        layer.msg(d.msg);
                    }
