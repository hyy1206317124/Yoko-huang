	...
	<form>
		<div class="row cl">
			...
			<div class="formControls col-xs-6 col-sm-6">
                            <input style="display:none;" type="text" class="input-text" value="<?php echo ($row['id']); ?>" placeholder="" id="id" name="id">
				<input type="text" class="input-text" value="<?php echo ($row['cate_title']); ?>" placeholder="" id="cate_title" name="cate_title">
			</div>
		</div>
...
$(function(){
	$('#sub').click(function(){
           var title = $('#cate_title').val();
           if(title.length == 0){
			layer.msg('分类名称不能为空',{icon: 5,time:1000});
                        return false;
            }
           var url="<?php echo U('Information/information_cate_doUpdate');?>";
           var data={};
           data['cate_title'] = $("#cate_title").val();
           data['id'] = $("#id").val();
           var dataType = 'json';
           $.post(
                url,
