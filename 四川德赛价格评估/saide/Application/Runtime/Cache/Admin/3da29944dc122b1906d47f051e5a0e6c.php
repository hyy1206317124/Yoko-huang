...
<!--	<div class="text-c">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20">
            <span class="l">
                ...
                <a class="btn btn-primary radius" onclick="picture_add('添加内容','<?php echo U('Column/column_add');?>')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加内容</a>
            </span> 
		...
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			<thead>
				...
				<tr class="text-c" id="ad_id<?php echo $ro['id'];?>">
					...
					<td class="text-l"><a class="maincolor" href="javascript:;" onClick="picture_edit('编辑','','10001')"><?php echo ($ro['title']); ?></a></td>
					<td><?php echo (date("Y-m-d H:i:s",$ro['time'])); ?></td>
                                       <?php if($ro['status'] == 1){ ?>
                                        ...
                                        <?php } ?>
					<td class="td-manage">
                                            <a style="text-decoration:none" class="ml-5" onClick="picture_edit('编辑','<?php echo U("Column/column_update?id=".$ro["id"]);?>','10001')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a> 
...
function picture_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: '<?php echo U("Column/column_delete");?>',
                        data:{id:id},
			dataType: 'json',
			success: function(data){
...
function datadel(){
    ...
    layer.confirm('确认要删除吗？',function(index){
        ...
        $.ajax({
            url:"<?php echo U('Column/column_delete_all');?>",
            data:{id:id_str},
            type:'post',
            dataType:'json',
