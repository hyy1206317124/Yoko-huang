...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 栏目管理 <span class="c-gray en">&gt;</span>栏目管理 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l">
                  <a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a>
                <a class="btn btn-primary radius" onclick="picture_add('添加栏目','<?php echo U('Column/yoko_class_add');?>')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加栏目</a>
            </span>
			...
			<thead>
				...
				<tr class="text-c" id="ad_id<?php echo $ro['id'];?>">
					...
					<td><input name="ad_id" type="checkbox" value="<?php echo ($ro['id']); ?>"></td>
                                   <?php }else{ ?>
                                         <td> <div style="     margin: 0 auto;width:15px; height:15px; background-color:#F00; border-radius:25px; ">  
                                            </div>  
                                         </td>
                                   <?php } ?>
					<td><?php echo ($ro['id']); ?></td>
                                        ...
                                        <td >
                                            <a href="<?php echo U('Column/read_lists?id='.$ro['id']);?>"><?php echo ($ro['class_title']); ?></a>
                                        </td>
					<td><?php echo (date("Y-m-d H:i:s",$ro['class_time'])); ?></td>
                                        <td>
                                            <a style="text-decoration:none" class="ml-5" onClick="picture_cate_add('增加子类','<?php echo U('Column/cate_add?id='.$ro['id']);?>')" href="javascript:;" title="增加子类"><i class="Hui-iconfont">&#xe6f3;</i></a>
                                            <a style="text-decoration:none" class="ml-5" onClick="picture_edit('栏目编辑','<?php echo U("Column/yoko_class_update?id=".$ro["id"]);?>','1','800','500')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>
                                             <?php if($ro['pid'] != 0){ ?>
                                            ...
                                            <?php }else{ ?>
                                            <a style="text-decoration:none" class="ml-5" onClick="deles(this,'<?php echo ($ro['id']); ?>')" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a>
                                            <?php } ?>
                                        </td>
				</tr>
...
function picture_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: '<?php echo U("Column/yoko_column_delete");?>',
                        data:{id:id},
			dataType: 'json',
...
/*-非删除*/
function deles(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		layer.msg('警告!非法操作，此项不能被删除!',{icon:5,time:3000});	
	});
}
/*批量删除*/
...
function datadel(){
    ...
    layer.confirm('确认要删除吗？',function(index){
        ...
        $.ajax({
            url:"<?php echo U('Column/yoko_column_delete_all');?>",
            data:{id:id_str},
            type:'post',
            dataType:'json',
