...
<?php if(is_array($infor)): $i = 0; $__LIST__ = $infor;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ce): $mod = ($i % 2 );++$i;?><title><?php echo ($ce['information_title']); ?>_<?php echo ($base['base_title']); ?></title><?php endforeach; endif; else: echo "" ;endif; ?>
	...
	<div class="con_sub_left" id="channel_scroll">
		<div class="serlist">
			...
			<div class="serlist_li">
				<ul>
                                      <?php if(is_array($read)): $i = 0; $__LIST__ = $read;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$co): $mod = ($i % 2 );++$i;?><li><a href="<?php echo U('Desai/Index/body?id='.$co['id']);?>" target="_blank"><?php echo ($co['title']); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
				</ul>
			</div>
...
<div>
	<span style="font-size:14px;">地址：<?php echo ($contact['contact_address']); ?></span></div>
	...
	<div class="con_sub_right">
		...
		<div class="con_sub_right_head">
                    <?php if(is_array($infor)): $i = 0; $__LIST__ = $infor;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ce): $mod = ($i % 2 );++$i;?><div class="con_sub_right_head_mark"></div>
			<div class="con_sub_right_head_name core"><?php echo ($ce['information_title']); ?></div><?php endforeach; endif; else: echo "" ;endif; ?> 
                        <div class="con_sub_right_head_more">
                            <span style="font-weight:bold;">当前位置: </span>
                            <a href="<?php echo U('Index/index');?>">主页</a> >
                        <a href='<?php echo U("Desai/Index/info?id=".$infot["uid"]);?>'><?php echo ($infot['cate_title']); ?></a> > 
                          </div>
                    </div>
                
		<div class="con_sub_right_con">
                    <?php if(is_array($infor)): $i = 0; $__LIST__ = $infor;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ro): $mod = ($i % 2 );++$i;?><h3><?php echo ($ro['information_title']); ?></h3>
			<div class="con_sub_right_con_con">
                            <span style="display: none;">&nbsp;</span><span style="display: none;">&nbsp;</span>
				<?php echo ($ro['information_body']); ?>
