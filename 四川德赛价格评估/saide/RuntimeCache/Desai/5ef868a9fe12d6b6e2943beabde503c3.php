	...
	<div class="con_sub_right">
		<div class="con_sub_right_head">
			...
			<span style="font-weight:bold;">当前位置：</span>
                        ...
                        <a href='<?php echo U("Desai/Index/read?id=".$bo["id"]);?>'><?php echo ($bo['class_title']); ?></a> ><?php endforeach; endif; else: echo "" ;endif; ?>
			</div>
		</div>
		<div class="con_sub_right_con">
			<ul>
                            <?php if(is_array($row)): $i = 0; $__LIST__ = $row;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$co): $mod = ($i % 2 );++$i;?><li class="con_sub_right_con_list">
					<a href="<?php echo U('Desai/Index/body?id='.$co['id'].'&'.'uid='.$co['uid']);?>" target="_blank"><?php echo ($co['title']); ?></a>
					<span><?php echo (time_date($co['time'] )); ?></span>
