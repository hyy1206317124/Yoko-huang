...
<?php if(is_array($body)): $i = 0; $__LIST__ = $body;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$bo): $mod = ($i % 2 );++$i;?><title><?php echo ($bo['class_title']); ?>_<?php echo ($base['base_title']); ?></title><?php endforeach; endif; else: echo "" ;endif; ?>
...
<script type="text/javascript">browserRedirect("http://m.scdesai.com");</script>
...
<div>
	...
	<div class="con_sub_right">
		<div class="con_sub_right_head">
                         <?php if(is_array($body)): $i = 0; $__LIST__ = $body;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$bo): $mod = ($i % 2 );++$i;?><div class="con_sub_right_head_mark"></div>
			<div class="con_sub_right_head_name"><?php echo ($bo['class_title']); ?></div>
			...
			<span style="font-weight:bold;">当前位置：</span>
                        ...
                        <a href='<?php echo U("Desai/Index/major?id=".$bo["id"]);?>'><?php echo ($bo['class_title']); ?></a> ><?php endforeach; endif; else: echo "" ;endif; ?>
			</div>
		</div>
		<div class="con_sub_right_con">
			<div class="con_sub_right_con_cus">
				<h4>我们的客户</h4>
				<dl>
                                    <?php if(is_array($major)): $i = 0; $__LIST__ = $major;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ma): $mod = ($i % 2 );++$i;?><dt>
                                    
						<img src="/saide.<?php echo ($ma['image']); ?>" width="100%" height="100%">
						<div class="con_sub_right_con_cus_name">
							<a href="<?php echo U('Index/major_body?id='.$ma['id'].'&'.'uid='.$ma['uid']);?>" target="_blank"><?php echo ($ma['title']); ?></a>
						</div>
					</dt><?php endforeach; endif; else: echo "" ;endif; ?>
				</dl>
			</div>
                     <div class="pages"><?php echo ($s); ?></div>
		</div>
	</div>
